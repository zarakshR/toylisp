...
enum { MPC_INPUT_STRING = 0, MPC_INPUT_FILE = 1, MPC_INPUT_PIPE = 2 };

enum { MPC_INPUT_MARKS_MIN = 32 };

enum { MPC_INPUT_MEM_NUM = 512 };
...
static int mpc_mem_ptr(mpc_input_t* i, void* p) {
    return (char*)p >= (char*)(i->mem)
           && (char*)p
                  < (char*)(i->mem) + (MPC_INPUT_MEM_NUM * sizeof(mpc_mem_t));
}
...
static void mpc_input_unmark(mpc_input_t* i) {
    ...
    if (i->marks_slots > i->marks_num + i->marks_num / 2
        && i->marks_slots > MPC_INPUT_MARKS_MIN) {
        i->marks_slots = i->marks_num > MPC_INPUT_MARKS_MIN
                             ? i->marks_num
                             : MPC_INPUT_MARKS_MIN;
        i->marks = realloc(i->marks, sizeof(mpc_state_t) * i->marks_slots);
        i->lasts = realloc(i->lasts, sizeof(char) * i->marks_slots);
...
static void mpc_input_rewind(mpc_input_t* i) {
    ...
    if (i->type == MPC_INPUT_FILE) { fseek(i->file, i->state.pos, SEEK_SET); }
...
static int mpc_input_success(mpc_input_t* i, char c, char** o) {
    ...
    if (i->type == MPC_INPUT_PIPE && i->buffer
...
static int mpc_input_range(mpc_input_t* i, char c, char d, char** o) {
    ...
    if (mpc_input_terminated(i)) { return 0; }
    x = mpc_input_getc(i);
    return x >= c && x <= d ? mpc_input_success(i, x, o)
...
static int mpc_input_oneof(mpc_input_t* i, const char* c, char** o) {
    ...
    if (mpc_input_terminated(i)) { return 0; }
    x = mpc_input_getc(i);
    return strchr(c, x) != 0 ? mpc_input_success(i, x, o)
                             : mpc_input_failure(i, x);
}
...
static void mpc_err_string_cat(char* buffer, int* pos, int* max,
                               char const* fmt, ...) {
...
static const char* mpc_err_char_unescape(char c, char char_unescape_buffer[4]) {
    ...
    case '\t': return "tab";
    case ' ': return "space";
    default: char_unescape_buffer[1] = c; return char_unescape_buffer;
    }
}
...
char* mpc_err_string(mpc_err_t* x) {
    ...
    if (x->failure) {
        mpc_err_string_cat(buffer, &pos, &max, "%s: error: %s\n", x->filename,
                           x->failure);
        return buffer;
    }

    mpc_err_string_cat(buffer, &pos, &max, "%s:%li:%li: error: expected ",
    ...
    mpc_err_string_cat(
        buffer, &pos, &max,
        mpc_err_char_unescape(x->received, char_unescape_buffer));
...
static int mpc_err_contains_expected(mpc_input_t* i, mpc_err_t* x,
                                     char* expected) {
    int j;
    (void)i;
    for (j = 0; j < x->expected_num; j++) {
...
static mpc_err_t* mpc_err_repeat(mpc_input_t* i, mpc_err_t* x,
                                 const char* prefix) {
    ...
    if (x->expected_num == 0) {
        ...
        x->expected_num = 1;
        x->expected =
            mpc_realloc(i, x->expected, sizeof(char*) * x->expected_num);
        x->expected[0] = expect;
        return x;
    }
...
static mpc_val_t* mpc_parse_fold(mpc_input_t* i, mpc_fold_t f, int n,
                                 mpc_val_t** xs) {
    int j;
...
static mpc_val_t* mpc_parse_apply_to(mpc_input_t* i, mpc_apply_to_t f,
                                     mpc_val_t* x, mpc_val_t* d) {
...
static int mpc_parse_run(mpc_input_t* i, mpc_parser_t* p, mpc_result_t* r,
                         mpc_err_t** e, int depth) {
    ...
    case MPC_TYPE_SINGLE:
        MPC_PRIMITIVE(mpc_input_char(i, p->data.single.x, (char**)&r->output));
    case MPC_TYPE_RANGE:
        MPC_PRIMITIVE(mpc_input_range(i, p->data.range.x, p->data.range.y,
                                      (char**)&r->output));
    case MPC_TYPE_ONEOF:
        MPC_PRIMITIVE(mpc_input_oneof(i, p->data.string.x, (char**)&r->output));
    case MPC_TYPE_NONEOF:
        MPC_PRIMITIVE(
            mpc_input_noneof(i, p->data.string.x, (char**)&r->output));
    case MPC_TYPE_SATISFY:
        MPC_PRIMITIVE(
            mpc_input_satisfy(i, p->data.satisfy.f, (char**)&r->output));
    case MPC_TYPE_STRING:
        MPC_PRIMITIVE(
            mpc_input_string(i, p->data.string.x, (char**)&r->output));
    case MPC_TYPE_ANCHOR:
        MPC_PRIMITIVE(
            mpc_input_anchor(i, p->data.anchor.f, (char**)&r->output));
    case MPC_TYPE_SOI: MPC_PRIMITIVE(mpc_input_soi(i, (char**)&r->output));
    case MPC_TYPE_EOI:
        MPC_PRIMITIVE(mpc_input_eoi(i, (char**)&r->output));
    ...
    case MPC_TYPE_STATE:
        MPC_SUCCESS(mpc_input_state_copy(i));
    ...
    case MPC_TYPE_APPLY_TO:
        if (mpc_parse_run(i, p->data.apply_to.x, r, e, depth + 1)) {
            MPC_SUCCESS(mpc_parse_apply_to(i, p->data.apply_to.f, r->output,
                                           p->data.apply_to.d));
        } else {
            MPC_FAILURE(r->error);
        }
    ...
    case MPC_TYPE_MANY:
        ...
        while (mpc_parse_run(i, p->data.repeat.x, &results[j], e, depth + 1)) {
            ...
            if (j == MPC_PARSE_STACK_MIN) {
                ...
                memcpy(results, results_stk,
                       sizeof(mpc_result_t) * MPC_PARSE_STACK_MIN);
            } else if (j >= results_slots) {
                results_slots = j + j / 2;
                results       = mpc_realloc(i, results,
                                            sizeof(mpc_result_t) * results_slots);
            }
        }
    ...
    case MPC_TYPE_COUNT:
        ...
        if (j == p->data.repeat.n) {
            MPC_SUCCESS(
                mpc_parse_fold(i, p->data.repeat.f, j, (mpc_val_t**)results);
                if (p->data.repeat.n > MPC_PARSE_STACK_MIN) {
                    mpc_free(i, results);
                });
        } else {
            for (k = 0; k < j; k++) {
            ...
            MPC_FAILURE(mpc_err_count(i, results[j].error, p->data.repeat.n);
                        if (p->data.repeat.n > MPC_PARSE_STACK_MIN) {
                            mpc_free(i, results);
                        });
        }
    ...
    case MPC_TYPE_OR:
        ...
        for (j = 0; j < p->data.or.n; j++) {
            if (mpc_parse_run(i, p->data.or.xs[j], &results[j], e, depth + 1)) {
                MPC_SUCCESS(results[j].output;
                            if (p->data.or.n > MPC_PARSE_STACK_MIN) {
                                mpc_free(i, results);
                            });
            } else {
                *e = mpc_err_merge(i, *e, results[j].error);
        ...
        MPC_FAILURE(NULL; if (p->data.or.n > MPC_PARSE_STACK_MIN) {
            mpc_free(i, results);
        });
    ...
    default: MPC_FAILURE(mpc_err_fail(i, "Unknown Parser Type Id!"));
    }
...
static void mpc_undefine_unretained(mpc_parser_t* p, int force) {
    ...
    case MPC_TYPE_ONEOF:
    case MPC_TYPE_NONEOF:
    case MPC_TYPE_STRING: free(p->data.string.x); break;
    ...
    case MPC_TYPE_MAYBE:
    case MPC_TYPE_NOT: mpc_undefine_unretained(p->data.not .x, 0); break;
    ...
    case MPC_TYPE_MANY:
    case MPC_TYPE_MANY1:
    case MPC_TYPE_COUNT: mpc_undefine_unretained(p->data.repeat.x, 0); break;
...
void mpc_delete(mpc_parser_t* p) {
    if (p->retained) {
        ...
        if (p->type != MPC_TYPE_UNDEFINED) { mpc_undefine_unretained(p, 0); }
...
mpc_parser_t* mpc_copy(mpc_parser_t* a) {
    ...
    case MPC_TYPE_APPLY_TO:
        p->data.apply_to.x = mpc_copy(a->data.apply_to.x);
        break;
    case MPC_TYPE_PREDICT:
        p->data.predict.x = mpc_copy(a->data.predict.x);
        break;

    case MPC_TYPE_MAYBE:
    case MPC_TYPE_NOT: p->data.not .x = mpc_copy(a->data.not .x); break;
    ...
    case MPC_TYPE_MANY:
    case MPC_TYPE_MANY1:
    case MPC_TYPE_COUNT: p->data.repeat.x = mpc_copy(a->data.repeat.x); break;
...
mpc_parser_t* mpc_or(int n, ...) {
    ...
    va_start(va, n);
    for (i = 0; i < n; i++) { p->data.or.xs[i] = va_arg(va, mpc_parser_t*); }
    va_end(va);
...
mpc_parser_t* mpc_and(int n, mpc_fold_t f, ...) {
    ...
    va_start(va, f);
    for (i = 0; i < n; i++) { p->data.and.xs[i] = va_arg(va, mpc_parser_t*); }
    for (i = 0; i < (n - 1); i++) {
...
mpc_parser_t* mpc_boundary(void) {
    return mpc_expect(mpc_anchor(mpc_boundary_anchor), "word boundary");
}
mpc_parser_t* mpc_boundary_newline(void) {
    return mpc_expect(mpc_anchor(mpc_boundary_newline_anchor),
                      "start of newline");
}

mpc_parser_t* mpc_whitespace(void) {
    return mpc_expect(mpc_oneof(" \f\n\r\t\v"), "whitespace");
}
mpc_parser_t* mpc_whitespaces(void) {
    return mpc_expect(mpc_many(mpcf_strfold, mpc_whitespace()), "spaces");
}
mpc_parser_t* mpc_blank(void) {
    return mpc_expect(mpc_apply(mpc_whitespaces(), mpcf_free), "whitespace");
}

mpc_parser_t* mpc_newline(void) {
    return mpc_expect(mpc_char('\n'), "newline");
}
mpc_parser_t* mpc_tab(void) {
    return mpc_expect(mpc_char('\t'), "tab");
}
mpc_parser_t* mpc_escape(void) {
    return mpc_and(2, mpcf_strfold, mpc_char('\\'), mpc_any(), free);
}

mpc_parser_t* mpc_digit(void) {
    return mpc_expect(mpc_oneof("0123456789"), "digit");
}
mpc_parser_t* mpc_hexdigit(void) {
    return mpc_expect(mpc_oneof("0123456789ABCDEFabcdef"), "hex digit");
}
mpc_parser_t* mpc_octdigit(void) {
    return mpc_expect(mpc_oneof("01234567"), "oct digit");
}
mpc_parser_t* mpc_digits(void) {
    return mpc_expect(mpc_many1(mpcf_strfold, mpc_digit()), "digits");
}
mpc_parser_t* mpc_hexdigits(void) {
    return mpc_expect(mpc_many1(mpcf_strfold, mpc_hexdigit()), "hex digits");
}
mpc_parser_t* mpc_octdigits(void) {
    return mpc_expect(mpc_many1(mpcf_strfold, mpc_octdigit()), "oct digits");
}

mpc_parser_t* mpc_lower(void) {
    return mpc_expect(mpc_oneof("abcdefghijklmnopqrstuvwxyz"),
                      "lowercase letter");
}
...
mpc_parser_t* mpc_alpha(void) {
    return mpc_expect(
        mpc_oneof("abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ"),
        "letter");
}
mpc_parser_t* mpc_underscore(void) {
    return mpc_expect(mpc_char('_'), "underscore");
}
mpc_parser_t* mpc_alphanum(void) {
    return mpc_expect(mpc_or(3, mpc_alpha(), mpc_digit(), mpc_underscore()),
                      "alphanumeric");
}

mpc_parser_t* mpc_int(void) {
    return mpc_expect(mpc_apply(mpc_digits(), mpcf_int), "integer");
}
mpc_parser_t* mpc_hex(void) {
    return mpc_expect(mpc_apply(mpc_hexdigits(), mpcf_hex), "hexadecimal");
}
mpc_parser_t* mpc_oct(void) {
    return mpc_expect(mpc_apply(mpc_octdigits(), mpcf_oct), "octadecimal");
}
mpc_parser_t* mpc_number(void) {
    return mpc_expect(mpc_or(3, mpc_int(), mpc_hex(), mpc_oct()), "number");
}
...
mpc_parser_t* mpc_real(void) {
    ...
    p1 = mpc_digits();
    p2 = mpc_maybe_lift(
        mpc_and(2, mpcf_strfold, mpc_char('.'), mpc_digits(), free),
        mpcf_ctor_str);
    p30 = mpc_oneof("eE");
    p31 = mpc_maybe_lift(mpc_oneof("+-"), mpcf_ctor_str);
    p32 = mpc_digits();
    p3  = mpc_maybe_lift(mpc_and(3, mpcf_strfold, p30, p31, p32, free, free),
                         mpcf_ctor_str);

    return mpc_expect(
        mpc_and(4, mpcf_strfold, p0, p1, p2, p3, free, free, free), "real");
}
...
mpc_parser_t* mpc_char_lit(void) {
    return mpc_expect(
        mpc_between(mpc_or(2, mpc_escape(), mpc_any()), free, "'", "'"),
        "char");
}
...
mpc_parser_t* mpc_string_lit(void) {
    ...
    return mpc_expect(
        mpc_between(mpc_many(mpcf_strfold, strchar), free, "\"", "\""),
        "string");
}
...
mpc_parser_t* mpc_regex_lit(void) {
    ...
    return mpc_expect(
        mpc_between(mpc_many(mpcf_strfold, regexchar), free, "/", "/"),
        "regex");
}
...
mpc_parser_t* mpc_startwith(mpc_parser_t* a) {
    return mpc_and(2, mpcf_snd, mpc_soi(), a, mpcf_dtor_null);
}
mpc_parser_t* mpc_endwith(mpc_parser_t* a, mpc_dtor_t da) {
    return mpc_and(2, mpcf_fst, a, mpc_eoi(), da);
}
mpc_parser_t* mpc_whole(mpc_parser_t* a, mpc_dtor_t da) {
    return mpc_and(3, mpcf_snd, mpc_soi(), a, mpc_eoi(), mpcf_dtor_null, da);
}

mpc_parser_t* mpc_stripl(mpc_parser_t* a) {
    return mpc_and(2, mpcf_snd, mpc_blank(), a, mpcf_dtor_null);
}
mpc_parser_t* mpc_stripr(mpc_parser_t* a) {
    return mpc_and(2, mpcf_fst, a, mpc_blank(), mpcf_dtor_null);
}
mpc_parser_t* mpc_strip(mpc_parser_t* a) {
    return mpc_and(3, mpcf_snd, mpc_blank(), a, mpc_blank(), mpcf_dtor_null,
                   mpcf_dtor_null);
}
...
mpc_parser_t* mpc_sym(const char* s) {
    return mpc_tok(mpc_string(s));
}

mpc_parser_t* mpc_total(mpc_parser_t* a, mpc_dtor_t da) {
    return mpc_whole(mpc_strip(a), da);
}

mpc_parser_t* mpc_between(mpc_parser_t* a, mpc_dtor_t ad, const char* o,
                          const char* c) {
    return mpc_and(3, mpcf_snd_free, mpc_string(o), a, mpc_string(c), free, ad);
}
...
mpc_parser_t* mpc_tok_between(mpc_parser_t* a, mpc_dtor_t ad, const char* o,
                              const char* c) {
    return mpc_and(3, mpcf_snd_free, mpc_sym(o), mpc_tok(a), mpc_sym(c), free,
                   ad);
}
...
static mpc_val_t* mpcf_re_and(int n, mpc_val_t** xs) {
    ...
    for (i = 0; i < n; i++) { p = mpc_and(2, mpcf_strfold, p, xs[i], free); }
    return p;
}
...
static mpc_val_t* mpcf_re_repeat(int n, mpc_val_t** xs) {
    ...
    case '*': {
        free(xs[1]);
        return mpc_many(mpcf_strfold, xs[0]);
    }; break;
    case '+': {
        free(xs[1]);
        return mpc_many1(mpcf_strfold, xs[0]);
    }; break;
    case '?': {
        free(xs[1]);
        return mpc_maybe_lift(xs[0], mpcf_ctor_str);
    }; break;
    default: num = *(int*)xs[1]; free(xs[1]);
    }
...
static mpc_parser_t* mpc_re_escape_char(char c) {
    ...
    case 'b':
        return mpc_and(2, mpcf_snd, mpc_boundary(), mpc_lift(mpcf_ctor_str),
                       free);
    case 'B': return mpc_not_lift(mpc_boundary(), free, mpcf_ctor_str);
    case 'A':
        return mpc_and(2, mpcf_snd, mpc_soi(), mpc_lift(mpcf_ctor_str), free);
    case 'Z':
        return mpc_and(2, mpcf_snd, mpc_eoi(), mpc_lift(mpcf_ctor_str), free);
...
static mpc_val_t* mpcf_re_escape(mpc_val_t* x, void* data) {
    ...
    if (s[0] == '^') {
        ...
        if (mode & MPC_RE_MULTILINE) {
            return mpc_and(2, mpcf_snd,
                           mpc_or(2, mpc_soi(), mpc_boundary_newline()),
                           mpc_lift(mpcf_ctor_str), free);
        } else {
            return mpc_and(2, mpcf_snd, mpc_soi(), mpc_lift(mpcf_ctor_str),
                           free);
        }
    }
    ...
    if (s[0] == '$') {
        ...
        if (mode & MPC_RE_MULTILINE) {
            ...
            return mpc_or(
                2, mpc_and(2, mpcf_fst, mpc_newline(), mpc_eoi(), free),
                mpc_and(2, mpcf_snd, mpc_eoi(), mpc_lift(mpcf_ctor_str), free));
        }
    }
...
static mpc_val_t* mpcf_re_range(mpc_val_t* x) {
    ...
    if (s[0] == '\0') {
        free(range);
        free(x);
        return mpc_fail("Invalid Regex Range Expression");
    }
    if (s[0] == '^' && s[1] == '\0') {
        free(range);
        free(x);
...
mpc_parser_t* mpc_re_mode(const char* re, int mode) {
    ...
    mpc_define(Regex, mpc_and(2, mpcf_re_or, Term,
    ...
    mpc_define(Factor,
               mpc_and(2, mpcf_re_repeat, Base,
                       mpc_or(5, mpc_char('*'), mpc_char('+'), mpc_char('?'),
                              mpc_brackets(mpc_int(), free), mpc_pass()),
                       (mpc_dtor_t)mpc_delete));

    mpc_define(Base,
               mpc_or(4, mpc_parens(Regex, (mpc_dtor_t)mpc_delete),
    ...
    mpc_define(Range, mpc_apply(mpc_many(mpcf_strfold, mpc_or(2, mpc_escape(),
                                                              mpc_noneof("]"))),
                                mpcf_re_range));
...
static mpc_val_t* mpcf_escape_new(mpc_val_t* x, const char* input,
                                  const char** output) {
...
static mpc_val_t* mpcf_unescape_new(mpc_val_t* x, const char* input,
                                    const char** output) {
    ...
    while (*s) {
        ...
        while (output[i]) {
            if ((*(s + 0)) == output[i][0] && (*(s + 1)) == output[i][1]) {
...
mpc_val_t* mpcf_unescape(mpc_val_t* x) {
    mpc_val_t* y =
        mpcf_unescape_new(x, mpc_escape_input_c, mpc_escape_output_c);
    free(x);
    return y;
}
...
mpc_val_t* mpcf_escape_regex(mpc_val_t* x) {
    mpc_val_t* y =
        mpcf_escape_new(x, mpc_escape_input_raw_re, mpc_escape_output_raw_re);
    free(x);
    return y;
}
...
mpc_val_t* mpcf_unescape_regex(mpc_val_t* x) {
    mpc_val_t* y =
        mpcf_unescape_new(x, mpc_escape_input_raw_re, mpc_escape_output_raw_re);
    free(x);
    return y;
}

mpc_val_t* mpcf_escape_string_raw(mpc_val_t* x) {
    mpc_val_t* y = mpcf_escape_new(x, mpc_escape_input_raw_cstr,
                                   mpc_escape_output_raw_cstr);
    free(x);
    return y;
}

mpc_val_t* mpcf_unescape_string_raw(mpc_val_t* x) {
    mpc_val_t* y = mpcf_unescape_new(x, mpc_escape_input_raw_cstr,
                                     mpc_escape_output_raw_cstr);
    free(x);
    return y;
}
...
mpc_val_t* mpcf_escape_char_raw(mpc_val_t* x) {
    mpc_val_t* y = mpcf_escape_new(x, mpc_escape_input_raw_cchar,
                                   mpc_escape_output_raw_cchar);
    free(x);
    return y;
}

mpc_val_t* mpcf_unescape_char_raw(mpc_val_t* x) {
    mpc_val_t* y = mpcf_unescape_new(x, mpc_escape_input_raw_cchar,
                                     mpc_escape_output_raw_cchar);
    free(x);
    return y;
}
...
static void mpc_print_unretained(mpc_parser_t* p, int force) {
    ...
    if (p->type == MPC_TYPE_SINGLE) {
        ...
        buff[1] = '\0';
        s = mpcf_escape_new(buff, mpc_escape_input_c, mpc_escape_output_c);
        printf("'%s'", s);
        free(s);
    }
    ...
    if (p->type == MPC_TYPE_RANGE) {
        ...
        buff[1] = '\0';
        e = mpcf_escape_new(buff, mpc_escape_input_c, mpc_escape_output_c);
        printf("[%s-%s]", s, e);
        free(s);
        free(e);
    }
    ...
    if (p->type == MPC_TYPE_ONEOF) {
        s = mpcf_escape_new(p->data.string.x, mpc_escape_input_c,
                            mpc_escape_output_c);
        printf("[%s]", s);
        free(s);
    }
    ...
    if (p->type == MPC_TYPE_APPLY_TO) {
        mpc_print_unretained(p->data.apply_to.x, 0);
    }
    if (p->type == MPC_TYPE_PREDICT) {
        mpc_print_unretained(p->data.predict.x, 0);
    }

    if (p->type == MPC_TYPE_NOT) {
        mpc_print_unretained(p->data.not .x, 0);
        printf("!");
    }
    ...
    if (p->type == MPC_TYPE_MANY) {
        mpc_print_unretained(p->data.repeat.x, 0);
        printf("*");
    }
...
mpc_ast_t* mpc_ast_build(int n, const char* tag, ...) {
    ...
    for (i = 0; i < n; i++) { mpc_ast_add_child(a, va_arg(va, mpc_ast_t*)); }
...
static void mpc_ast_print_depth(mpc_ast_t* a, int d, FILE* fp) {
    ...
    if (strlen(a->contents)) {
        fprintf(fp, "%s:%lu:%lu '%s'\n", a->tag,
                ...
                (long unsigned int)(a->state.col + 1), a->contents);
    } else {
        fprintf(fp, "%s \n", a->tag);
    }
...
int mpc_ast_get_index_lb(mpc_ast_t* ast, const char* tag, int lb) {
    ...
    for (i = lb; i < ast->children_num; i++) {
        if (strcmp(ast->children[i]->tag, tag) == 0) { return i; }
    }
...
mpc_ast_t* mpc_ast_traverse_next(mpc_ast_trav_t** trav) {
    ...
    case mpc_ast_trav_order_post:
        ...
        *trav   = (*trav)->parent;
        ...
        if ((*trav)->curr_child >= (*trav)->curr_node->children_num) { break; }
...
mpc_val_t* mpcf_fold_ast(int n, mpc_val_t** xs) {
    ...
    for (i = 0; i < n; i++) {
        ...
        if (as[i] && as[i]->children_num == 0) {
            ...
            mpc_ast_add_child(
                r, mpc_ast_add_root_tag(as[i]->children[0], as[i]->tag));
...
mpc_parser_t* mpca_not(mpc_parser_t* a) {
    return mpc_not(a, (mpc_dtor_t)mpc_ast_delete);
}
mpc_parser_t* mpca_maybe(mpc_parser_t* a) {
    return mpc_maybe(a);
}
mpc_parser_t* mpca_many(mpc_parser_t* a) {
    return mpc_many(mpcf_fold_ast, a);
}
mpc_parser_t* mpca_many1(mpc_parser_t* a) {
    return mpc_many1(mpcf_fold_ast, a);
}
mpc_parser_t* mpca_count(int n, mpc_parser_t* a) {
    return mpc_count(n, mpcf_fold_ast, a, (mpc_dtor_t)mpc_ast_delete);
}
...
mpc_parser_t* mpca_total(mpc_parser_t* a) {
    return mpc_total(a, (mpc_dtor_t)mpc_ast_delete);
}
...
static mpc_val_t* mpcaf_grammar_repeat(int n, mpc_val_t** xs) {
    ...
    case '*': {
        free(xs[1]);
        return mpca_many(xs[0]);
    }; break;
    case '+': {
        free(xs[1]);
        return mpca_many1(xs[0]);
    }; break;
    case '?': {
        free(xs[1]);
        return mpca_maybe(xs[0]);
    }; break;
    case '!': {
        free(xs[1]);
...
static int is_number(const char* s) {
    ...
    for (i = 0; i < strlen(s); i++) {
        if (!strchr("0123456789", s[i])) { return 0; }
    }
    return 1;
}
    ...
    if (is_number(x)) {
        ...
        while (st->parsers_num <= i) {
            ...
            if (st->parsers[st->parsers_num - 1] == NULL) {
                return mpc_failf(
                    "No Parser in position %i! Only supplied %i Parsers!", i,
                    st->parsers_num);
            }
        }
...
mpc_parser_t* mpca_grammar_st(const char* grammar, mpca_grammar_st_t* st) {
    ...
    mpc_define(Grammar, mpc_and(2, mpcaf_grammar_or, Term,
                                mpc_maybe(mpc_and(2, mpcf_snd_free,
                                                  mpc_sym("|"), Grammar, free)),
                                mpc_soft_delete));
    ...
    mpc_define(Factor,
               mpc_and(2, mpcaf_grammar_repeat, Base,
                       mpc_or(6, mpc_sym("*"), mpc_sym("+"), mpc_sym("?"),
                              mpc_sym("!"), mpc_tok_brackets(mpc_int(), free),
                              mpc_pass()),
                       mpc_soft_delete));
    ...
    return (st->flags & MPCA_LANG_PREDICTIVE) ? mpc_predictive(r.output)
                                              : r.output;
}
...
static mpc_err_t* mpca_lang_st(mpc_input_t* i, mpca_grammar_st_t* st) {
    ...
    mpc_define(
        Lang,
        mpc_apply_to(mpc_total(mpc_predictive(mpc_many(mpca_stmt_fold, Stmt)),
                               mpca_stmt_list_delete),
                     mpca_stmt_list_apply_to, st));

    mpc_define(Stmt, mpc_and(5, mpca_stmt_afold, mpc_tok(mpc_ident()),
                             mpc_maybe(mpc_tok(mpc_string_lit())), mpc_sym(":"),
                             Grammar, mpc_sym(";"), free, free, free,
                             mpc_soft_delete));
...
static int mpc_nodecount_unretained(mpc_parser_t* p, int force) {
    ...
    if (p->type == MPC_TYPE_EXPECT) {
        return 1 + mpc_nodecount_unretained(p->data.expect.x, 0);
    }

    if (p->type == MPC_TYPE_APPLY) {
        return 1 + mpc_nodecount_unretained(p->data.apply.x, 0);
    }
    if (p->type == MPC_TYPE_APPLY_TO) {
        return 1 + mpc_nodecount_unretained(p->data.apply_to.x, 0);
    }
    if (p->type == MPC_TYPE_PREDICT) {
        return 1 + mpc_nodecount_unretained(p->data.predict.x, 0);
    }

    if (p->type == MPC_TYPE_CHECK) {
        return 1 + mpc_nodecount_unretained(p->data.check.x, 0);
    }
    if (p->type == MPC_TYPE_CHECK_WITH) {
        return 1 + mpc_nodecount_unretained(p->data.check_with.x, 0);
    }

    if (p->type == MPC_TYPE_NOT) {
        return 1 + mpc_nodecount_unretained(p->data.not .x, 0);
    }
    if (p->type == MPC_TYPE_MAYBE) {
        return 1 + mpc_nodecount_unretained(p->data.not .x, 0);
    }
    ...
    if (p->type == MPC_TYPE_MANY) {
        return 1 + mpc_nodecount_unretained(p->data.repeat.x, 0);
    }
    if (p->type == MPC_TYPE_MANY1) {
        return 1 + mpc_nodecount_unretained(p->data.repeat.x, 0);
    }
...
static void mpc_optimise_unretained(mpc_parser_t* p, int force) {
    ...
    if (p->type == MPC_TYPE_APPLY) {
        mpc_optimise_unretained(p->data.apply.x, 0);
    }
    if (p->type == MPC_TYPE_APPLY_TO) {
        mpc_optimise_unretained(p->data.apply_to.x, 0);
    }
    if (p->type == MPC_TYPE_CHECK) {
        mpc_optimise_unretained(p->data.check.x, 0);
    }
    if (p->type == MPC_TYPE_CHECK_WITH) {
        mpc_optimise_unretained(p->data.check_with.x, 0);
    }
    if (p->type == MPC_TYPE_PREDICT) {
        mpc_optimise_unretained(p->data.predict.x, 0);
    }
    if (p->type == MPC_TYPE_NOT) { mpc_optimise_unretained(p->data.not .x, 0); }
    if (p->type == MPC_TYPE_MAYBE) {
        mpc_optimise_unretained(p->data.not .x, 0);
    }
    if (p->type == MPC_TYPE_MANY) {
    ...
    while (1) {
        ...
        /* Merge rhs `or` */
        if (p->type == MPC_TYPE_OR && p->data.
            or.xs[p->data.or.n - 1]->type == MPC_TYPE_OR && !p->data.
            or.xs[p->data.or.n - 1]->retained) {
            t            = p->data.or.xs[p->data.or.n - 1];
            n            = p->data.or.n;
            m            = t->data.or.n;
            p->data.or.n = n + m - 1;
            p->data.or.xs =
                realloc(p->data.or.xs, sizeof(mpc_parser_t*) * (n + m - 1));
            memmove(p->data.or.xs + n - 1, t->data.or.xs,
                    m * sizeof(mpc_parser_t*));
            free(t->data.or.xs);
            free(t->name);
            free(t);
            continue;
        ...
        /* Merge lhs `or` */
        if (p->type == MPC_TYPE_OR && p->data.
            or.xs[0]->type == MPC_TYPE_OR && !p->data.or.xs[0]->retained) {
            t            = p->data.or.xs[0];
            n            = p->data.or.n;
            m            = t->data.or.n;
            p->data.or.n = n + m - 1;
            p->data.or.xs =
                realloc(p->data.or.xs, sizeof(mpc_parser_t*) * (n + m - 1));
            memmove(p->data.or.xs + m, p->data.or.xs + 1,
                    (n - 1) * sizeof(mpc_parser_t*));
        ...
        /* Remove ast `pass` */
        if (p->type == MPC_TYPE_AND && p->data.and.n == 2 && p->data.
            and.xs[0]->type == MPC_TYPE_PASS && !p->data.and.xs[0]->retained
            && p->data.and.f == mpcf_fold_ast) {
            t = p->data.and.xs[1];
        ...
        /* Merge ast lhs `and` */
        if (p->type == MPC_TYPE_AND && p->data.and.f == mpcf_fold_ast
            && p->data.and.xs[0]->type == MPC_TYPE_AND && !p->data.
            and.xs[0]->retained && p->data.and.xs[0]->data.
            and.f == mpcf_fold_ast) {
            t             = p->data.and.xs[0];
            n             = p->data.and.n;
            m             = t->data.and.n;
            p->data.and.n = n + m - 1;
            p->data.and.xs =
                realloc(p->data.and.xs, sizeof(mpc_parser_t*) * (n + m - 1));
            p->data.and.dxs =
                realloc(p->data.and.dxs, sizeof(mpc_dtor_t) * (n + m - 1 - 1));
            memmove(p->data.and.xs + m, p->data.and.xs + 1,
                    (n - 1) * sizeof(mpc_parser_t*));
        ...
        if (p->type == MPC_TYPE_AND && p->data.and.f == mpcf_fold_ast
            ...
            && !p->data.and.xs[p->data.and.n - 1]->retained && p->data.
            and.xs[p->data.and.n - 1]->data.and.f == mpcf_fold_ast) {
            ...
            p->data.and.dxs =
                realloc(p->data.and.dxs, sizeof(mpc_dtor_t) * (n + m - 1 - 1));
            memmove(p->data.and.xs + n - 1, t->data.and.xs,
                    m * sizeof(mpc_parser_t*));
            for (i = 0; i < p->data.and.n - 1; i++) {
                p->data.and.dxs[i] = (mpc_dtor_t)mpc_ast_delete;
            }
        ...
        /* Remove re `lift` */
        if (p->type == MPC_TYPE_AND && p->data.and.n == 2 && p->data.
            and.xs[0]->type == MPC_TYPE_LIFT && p->data.
            and.xs[0]->data.lift.lf == mpcf_ctor_str && !p->data.
            and.xs[0]->retained && p->data.and.f == mpcf_strfold) {
            t = p->data.and.xs[1];
            mpc_delete(p->data.and.xs[0]);
            free(p->data.and.xs);
            free(p->data.and.dxs);
        ...
        /* Merge re lhs `and` */
        if (p->type == MPC_TYPE_AND && p->data.and.f == mpcf_strfold && p->data.
            and.xs[0]->type == MPC_TYPE_AND && !p->data.and.xs[0]->retained
        ...
        /* Merge re rhs `and` */
        if (p->type == MPC_TYPE_AND && p->data.and.f == mpcf_strfold && p->data.
            and.xs[p->data.and.n - 1]->type == MPC_TYPE_AND && !p->data.
            and.xs[p->data.and.n - 1]->retained && p->data.
            and.xs[p->data.and.n - 1]->data.and.f == mpcf_strfold) {
            t             = p->data.and.xs[p->data.and.n - 1];
            n             = p->data.and.n;
            m             = t->data.and.n;
            p->data.and.n = n + m - 1;
